...
from RotCurves.plotting import (
    figure,
    COLORS
)
...
class MCMC_fitter:
    ...
    def _unpack_models_from_samples(self, samples):
        ...
        for theta in samples[np.random.randint(len(samples), size=self.nwalkers)]:
            ...
            mcmc_rotation_curves.append(RC.Vobs_sini)
    ...
    def loglike(self, theta):
        ...
        if self.galaxy.fit_goals['velocity']:
            prob = self._update_loglike(
                ...
                xinterp=self.galaxy.radial_space["array"], yinterp=RC.Vobs_sini
            )
    ...
    def full_mcmc_run(self):
        ...
        bestfit_RC = RotationCurveObject(
            ...
            sigma_beam=self.galaxy.sigma_beam,
            include_beam_smearing=True)
    ...
    def _save_fit_profiles(self, RC):
        ...
        interpolator = CubicSpline(x=Rarray, y=RC.Vobs_sini)
        ...
        out_df_intrinsic['v_circ'] = RC.Vcirc
        out_df_intrinsic['v_rot'] = RC.Vrot
    ...
    def _red_chisq(self, RC):
        ...
        chisq_vel = 0
        if self.galaxy.fit_goals['velocity']:
            interpolator_vel = CubicSpline(x=R_array, y=RC.Vobs_sini)
            ...
            if self.galaxy.fit_goals[fit_goal]:
                ...
                if fit_goal == 'flux':
                    ...
                    self.plot_single_bestfit(
                        ...
                        model_x=R_array, model_y=RC.Vobs_sini,
                        ax_values=axes[i], ax_res=axes[i + ncols]
                    )
    ...
    def plot_intrinsicRC(self, RC):
        ...
        # ax.plot(R_array, RC.Vobs_sini, "-", lw=2, color=red, label="$V_{obs}$")
        x = RC.R_majoraxis
        ax.plot(x, RC.Vrot, "-", lw=2, color=red, label="$V_{rot}$")
        ax.plot(x, RC.Vcirc, "-", lw=2, color=blue, label="$V_{circ}$")
